        ...
        <>
            <div className="relative rounded-xl overflow-auto p-0">
                <div className="grid grid-cols-4 divide-x  p-1  border-black ">
                    <div className="rounded-full hover:bg-gray-300 p-0" onClick={firstTab}>
                        <div className="pl-8" >
                            <label className="block text-gray-700 text-sm " >
                                Where
                            </label>
                            <input id="where" type="text" placeholder="Search Destination" className="border-none p-0 bg-transparent  focus:border-none focus:outline-none focus:border-0" />
                        </div>
                    </div>
                    {experiencetab === 0 && <><div className="rounded-full hover:bg-gray-300 p-0" onClick={secondTab}>
                        <div className="pl-8" >
                            <label className="block text-gray-700 text-sm " >
                                Check In
                            </label>
                            <span>{pickedDates.firstPickedDate ? pickedDates.firstPickedDate.toLocaleDateString() : "Add date"}</span>
                        </div>
                    </div>
                        <div className="rounded-full hover:bg-gray-300 p-0" onClick={thirdTab}>
                            <div className="pl-8" >
                                <label className="block text-gray-700 text-sm " >
                                    Check Out
                                </label>
                                <span>{pickedDates.secondPickedDate ? pickedDates.firstPickedDate.toLocaleDateString() : "Add date"}</span>
                            </div>
                        </div>
                    </>
                    }
                    {experiencetab === 1 && <div className="rounded-full hover:bg-gray-300 p-0" onClick={secondTab}>
                        <div className="pl-8" >
                            <label className="block text-gray-700 text-sm " >
                                Add date
                            </label>
                            <span>{pickedDates.firstPickedDate ? pickedDates.firstPickedDate.toLocaleDateString() : "Add date"} {pickedDates.secondPickedDate ? " - " + pickedDates.secondPickedDate.toLocaleDateString() : ''} </span>
                        </div>
                    </div>}

                    <div className="rounded-full hover:bg-gray-300 p-0" onClick={forthTab}>
            ...
            <div className='absolute z-10'>
                <div>
                    ...
                    {tabindex === 3 && <GuestCountTab guestcount={guestcount}></GuestCountTab> }
